...
camaras %>% filter(bloque %in% sprintf("B%02i",1:6)) %>%
  ...
  summarise(lat=mean(lat), lon=mean(lon), bloque=unique(bloque),
    ...
    bsq=mean(buf.fragmen), ndvi=mean(ndvi.mu), fecha1=min(fecha1),
    fecha2=max(fecha2)) %>%
...
```{r}
ggplot(cam.data, aes(x=grp, y=bsq)) +
    geom_boxplot(notch=F) + # or notch=T
    labs(title="Tree cover around the camera sites for each habitat type (1km)") +
    labs(y='% tree cover', x="habitats",caption="Tree cover from Hansen et al. (2013)")
```
...
```{r}
cam.data %>% select(bsq,ndvi,H,h) %>% cor()
...
```{r}
d1 <- pointDistance(cam.data[,c("lon","lat")], coordinates(conucos)[,1:2], lonlat=T,allpairs=T)
...
ggplot(data=cam.data, aes(y=dhum,x=bsq,colour=hunting,size=duration)) +
geom_point() +
...
eventos %>% mutate(f1 = chron(dates.=sprintf("%s-%s-%s",ano,mes,dia),
  ...
  transmute(cdg, camara, fotos, species=as.character(species), number.of.animals, fecha=chron(ifelse(is.na(f2),f1,f2),
      format = c(dates = "y-m-d", times = "h:m:s"))) %>%
...
```{r}
sC <- data.frame(cam.data[match(rownames(obs),cam.data$cdg),c("bloque","H","h","dcon","dcom","wcon","dhum","bsq","ndvi","grp","dras","hunting")])
...
```{r}
for (k in c("H","h","dcon","dras","ndvi","bsq","dcom","wcon","dhum")) {
...
We fit a null model using the blocks as covariates but the model is not identifiable:

```{r}
occuRN(~ bloque+sfrz ~ bloque, UMF,K=50)
```
...
fm11 <- occuRN(~ H+h+sfrz+date+dras ~ H+h+grp+bsq, UMF,K=50)

fm01 <- occuRN(~ H+h+sfrz ~ H+h+grp, UMF,K=50)
fm02 <- occuRN(~ H+h+date+sfrz ~ H+h+grp, UMF,K=50)
fm03 <- occuRN(~ H+h+date+dras+sfrz ~ H+h+grp, UMF,K=50)
fm04 <- occuRN(~ date+dras+sfrz ~ H+h+grp, UMF,K=50)

model.sel(list(fm01,fm02,fm03,fm04))

fm11 <- occuRN(~ H+h+sfrz ~ H+h, UMF,K=50)
fm12 <- occuRN(~ H+h+sfrz ~ H+h+grp, UMF,K=50)
fm13 <- occuRN(~ H+h+sfrz ~ bsq, UMF,K=50)
fm14 <- occuRN(~ H+h+sfrz ~ ndvi, UMF,K=50)

model.sel(list(fm11,fm12,fm13,fm14))

fm21 <- occuRN(~ date+sfrz+dras ~ H+h, UMF,K=50)
fm22 <- occuRN(~ date+sfrz+dras ~ H+h+grp, UMF,K=50)
fm23 <- occuRN(~ date+sfrz+dras ~ bsq, UMF,K=50)
fm24 <- occuRN(~ date+sfrz+dras ~ ndvi, UMF,K=50)

model.sel(list(fm21,fm22,fm23,fm24))

fm12 <- occuRN(~ date+sfrz+dras ~ bsq+dcon, UMF,K=50)
fm13 <- occuRN(~ date+sfrz+dras ~ bsq+wcon, UMF,K=50)
fm14 <- occuRN(~ date+sfrz+dras ~ bsq+dhum, UMF,K=50)

AIC(fm00,fm01,fm10,fm11,fm12,fm13,fm14)
```

```{r}
(fm01 <- occuRN(~ dras+date+sfrz ~ H+h, UMF,K=50))
