...
Symptoms of lack of fit for most species: Five models with extreme p-value or c-hat values, 18 models with large coeficients or standard errors.
...
for (spp in c('D.leporina','L.pardalis','C.alector','C.thous','P.onca','D.novemcinctus','M.gouazoubira','D.kappleri','C.paca','L.rufaxilla')) {
  arch <- sprintf("%s/Rdata/occuRN/%s.rda",script.dir,spp)
  lst <- (load(arch))
  print(spp)
...
plot(ss$coef.mavg.,1:nrow(ss),xlim=c(-2.3,2.3),pch=19,xlab='',ylab='',axes=F,main='distance to conuco')
segments(ss$X2.5..,   1:nrow(ss),   ss$X97.5..,1:nrow(ss))
axis(1)
axis(2,1:nrow(ss),ss$spp,las=2)
box()
...
load(arch)
mavg <- model.avg(oms03, subset = delta < 10,fit=T)
summary(mavg)
...
prd <- predict(mavg,type='state')
x <- UMF@siteCovs$caza.celda>0
a1 <- prd$fit
lm01 <- glm(x ~ dcon,data=UMF@siteCovs,family=binomial)
lm02 <- glm(x ~ a1,data=UMF@siteCovs,family=binomial)
