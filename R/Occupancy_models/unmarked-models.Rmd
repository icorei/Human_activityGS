...
These species appear to have  a good fit but the model has over-dispersion ($c-hat >1$), and might have problems with large coeficients and standard errors
```{r }
...
Significant conditional coeficients for p(sfrz). Positive ("atracted") but non-significant effect of conucos
...
### C. paca
Very strong support for p(sfrz)+lam(evi.mu)+lam(I(evi.mu^2)), and strong support for p(dras), but only 0.23 for wcon.
...
Significant conditional coeficients for those parameters. Very weak and non-significative effect of conucos
